...
Class MainClass{
    ...
    function __construct(){
        $this->db = new mysqli('localhost','root','','bookingcalendar');
        if(!$this->db){
    ...
    public function register(){
        ...
        $check = $this->db->query("SELECT * FROM `registered_users` where `email`= '$email' ")->num_rows;
        if($check > 0){
            $resp['status'] = 'failed';
            $_SESSION['flashdata']['type']='danger';
            $_SESSION['flashdata']['msg'] = ' Email already exists.';
        }else{
            $sql = "INSERT INTO `registered_users` (firstName,middleInitial,lastName,email,contact_number,address,dateofbirth,gender,`password`,confirm_password) 
                            VALUES ('$firstName','$middleInitial','$lastName','$email','$contact_number','$address','$dateofbirth','$gender','$password','$confirm_password')";
    ...
    public function login(){
        ...
        if($result->num_rows > 0){
            ...
            if($pass_is_right && (is_null($data['otp']) || (!is_null($data['otp']) && !is_null($data['otp_expiration']) && strtotime($data['otp_expiration']) < time()) ) ){
                ...
                $update_sql = "UPDATE `registered_users` set otp_expiration = '{$expiration}', otp = '{$otp}' where id='{$data['id']}' ";
    ...
    public function resend_otp($id){
        $otp = sprintf("%'.06d",mt_rand(0,999999));
        $expiration = date("Y-m-d H:i" ,strtotime(date('Y-m-d H:i')." +1 mins"));
        $update_sql = "UPDATE `registered_users` set otp_expiration = '{$expiration}', otp = '{$otp}' where id = '{$id}' ";
        $update_otp = $this->db->query($update_sql);
        if($update_otp){
            $resp['status'] = 'success';
            $email = $this->db->query("SELECT email FROM `registered_users` where id = '{$id}'")->fetch_array()[0];
    ...
    public function otp_verify(){
        extract($_POST);
         $sql = "SELECT * FROM `registered_users` where id = ? and otp = ?";
         $stmt = $this->db->prepare($sql);
         ...
         if($result->num_rows > 0){
             $resp['status'] = 'success';
             $this->db->query("UPDATE `registered_users` set otp = NULL, otp_expiration = NULL where id = '{$id}'");
